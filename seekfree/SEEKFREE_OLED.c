...
void oled_wrdat(uint8 data)
{
	...
	OLED_DC(1);
	 spi_mosi(OLED_SPI,OLED_CS_PIN,&data,&data,1,1);
}
...
void oled_wrcmd(uint8 cmd)
{
	...
	spi_mosi(OLED_SPI,OLED_CS_PIN,&cmd,&cmd,1,1);		
}
...
void oled_init(void)
{
	...
	OLED_PinInit();
(void)spi_init(OLED_SPI,OLED_CS_PIN,OLED_SCL_PIN,OLED_SDA_PIN, OLED_SDA_IN,0,10*1000*1000);
//	OLED_SCL(1);
	OLED_RST(0);
	systick_delay_ms(50);
	OLED_RST(1);
...
void OLED_P6x8Int(uint8_t x, uint8_t y, int16_t data1, int8 set)
{
  ...
  if(set < 0)
  {
    ...
    oled_p6x8str(x, y, (uint8 *)&(result[5 + set]));
  }
  else if(set > 0)
  {
    oled_p6x8str(x, y, (uint8 *)&(result[6 - set]));
  }
}
