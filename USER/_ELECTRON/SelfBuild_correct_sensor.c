...
{
  MCP41_Init();  
	gpio_init(CS0, GPO, 1,PULLUP);
	gpio_init(CS1, GPO, 1,PULLUP);
	gpio_init(CS2, GPO, 1,PULLUP);
	gpio_init(CS3, GPO, 1,PULLUP);
	gpio_init(CS4, GPO, 1,PULLUP);
  EEPROM_InitMCP41();
...
void Normalized_MCP41(void)
{
	...
	OLED_P6x8Str((Max_Column - 1 - 6 * (sizeof("Normalized") - 1)) / 2, 0, "Normalized");
	...
	while(1)
	{
		...
		if(gpio_get(Button_Up) == 0)//如果按下了上键，那么将设定的电阻值全部保存到FLASH
		{
			...
			for(int8 temp = 0; temp < 8; temp++)
				my_eeprom_write_word(FLASH_SAVE_MCP41, temp, MCP41_EEPROM_Save[temp]);//写入电阻值
			break;
		}
...
void EEPROM_InitMCP41(void)
{
  for(int8 temp = 0; temp < 8; temp++)
  {
          MCP41_EEPROM_Save[temp] =my_EEPROM_READ_WORD(FLASH_SAVE_MCP41,temp, uint8);
  }
