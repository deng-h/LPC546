...
void ctimer_clock(CTIMER_enum ctimern)    //开启时钟
{
    switch(ctimern)
    {
        case TIMER0:
...
void ctimer_pwmmux(CTIMER_PWMCH_enum pwmch)		//设置引脚复用功能->PWM
{
    switch(pwmch/8)
    {
        case TIMER0:
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      设置输入捕捉模式复用功能
//  @param      capturech    输入捕捉通道号及引脚
//  @return     void      
//  Sample usage:           
//-------------------------------------------------------------------------------------------------------------------
void ctimer_capturemux(CTIMER_CAPTURE_enum capturech)		//设置引脚复用功能->输入捕捉
{
		uint8 TIMER_CH;
...
void ctimer_capture_init(CTIMER_CAPTURE_enum capturech,CAPTURE_ch_enum CAPTURE_ch,CAPTURE_Input_cfg_enum cfg,CAPTURE_PS_e_enum ps)
{
	  ...
	  ctimer_clock(TIMER_CH); 	//开启时钟
		ctimer_capturemux(capturech);	//开启复用功能

    //TIMER寄存器配置
    TIMERN[TIMER_CH]->TCR  &= ~CTIMER_TCR_CEN_MASK;	//关闭定时器
		switch(cfg)
    {
    case CAPTURE_RISING:    //上升沿触发
				TIMERN[TIMER_CH]->CCR  =1<<(CAPTURE_ch*3) | 4<<(CAPTURE_ch*3);	//开启上升沿触发并触发中断
				TIMERN[TIMER_CH]->CTCR =CTIMER_CTCR_CTMODE(0) | CTIMER_CTCR_CINSEL(CAPTURE_ch)| CTIMER_CTCR_SELCC(CAPTURE_ch*2);
        break;

    case CAPTURE_FALLING:   //下降沿触发
				TIMERN[TIMER_CH]->CCR =2<<(CAPTURE_ch*3) | 4<<(CAPTURE_ch*3);	//开启下降沿沿触发并触发中断
				TIMERN[TIMER_CH]->CTCR =CTIMER_CTCR_CTMODE(0) | CTIMER_CTCR_CINSEL(CAPTURE_ch)| CTIMER_CTCR_SELCC(CAPTURE_ch*2+1);
        break;

    case CAPTURE_BOTH: //上升沿、下降沿都触发
        TIMERN[TIMER_CH]->CCR  =1<<(CAPTURE_ch*3) | 2<<(CAPTURE_ch*3) | 4<<(CAPTURE_ch*3);	//开启上升沿和下降沿触发并触发中断
				TIMERN[TIMER_CH]->CTCR =CTIMER_CTCR_CTMODE(0) | CTIMER_CTCR_CINSEL(CAPTURE_ch) | CTIMER_CTCR_SELCC(CAPTURE_ch*2+1);
        break;
    }
		TIMERN[TIMER_CH]->CTCR |= CTIMER_CTCR_ENCC_MASK;	//使能上升沿或下降沿触发清零定时器寄存器
    TIMERN[TIMER_CH]->PR = (ps==0?0:1)<<ps;     //设置预分频寄存器
		TIMERN[TIMER_CH]->IR = 1<<(CAPTURE_ch+4);	//清除中断标志位
    TIMERN[TIMER_CH]->TCR  = CTIMER_TCR_CRST_MASK;	//复位定时器
}
...
void capture_SetInput_cfg(CTIMER_CAPTURE_enum capturech,CAPTURE_ch_enum CAPTURE_ch,CAPTURE_Input_cfg_enum cfg)
{
    ...
    {
    case CAPTURE_RISING:    //上升沿触发
				TIMERN[TIMER_CH]->CCR &=~(2<<(CAPTURE_ch*3));	//关闭下降沿触发
				TIMERN[TIMER_CH]->CCR  =1<<(CAPTURE_ch*3);		//开启上升沿触发
        break;
    case CAPTURE_FALLING:   //下降沿触发
				TIMERN[TIMER_CH]->CCR &=~(1<<(CAPTURE_ch*3));	//关闭上升沿触发
				TIMERN[TIMER_CH]->CCR  =2<<(CAPTURE_ch*3);		//开启下降沿触发
        break;
    case CAPTURE_BOTH: //上升沿、下降沿都触发
        TIMERN[TIMER_CH]->CCR |=1<<(CAPTURE_ch*3) | 2<<(CAPTURE_ch*3);	//开启上升沿和下降沿触发
        break;
    }
}

void capture_enable_irq(CTIMER_CAPTURE_enum capturech)			//使能输入捕捉中断
{
	uint8 TIMER_CH;
	if(capturech<=7)		     TIMER_CH=TIMER0;
	else if(capturech<=11)   TIMER_CH=TIMER1;
...
void capture_disable_irq(CTIMER_CAPTURE_enum capturech)		//失能输入捕捉中断
{
	uint8 TIMER_CH;
	if(capturech<=7)		     TIMER_CH=TIMER0;
	else if(capturech<=11)   TIMER_CH=TIMER1;
